...
atualizar();
[12:42, 01/06/2026] Renata Rabêlo: let movimentos = JSON.parse(localStorage.getItem("financeiro")) || [];
...
function atualizar() {
  ...
  document.getElementById("ganhosMes").innerText = "R$ " + ganhosMes.toFixed(2);
  document.getElementById("gastosMes").innerText = "R$ " + gastosMes.toFixed(2);
  document.getElementById("saldoTotal").innerText = "R$ " + (ganhosMes - gastosMes).toFixed(2);

  // 📅 MÊS NA TELA (AGORA FUNCIONA)
  const titulo = document.getElementById("tituloMes");
  if (titulo) {
